...
const multerStorage = multer.diskStorage({
  ...
  filename: (req, file, callback) => {
    ...
    //! garantirame deka nema da ima poveke sliki so isto ime
    const ext = file.mimetype.split("/")[1];
    callback(null, `movie-${imageId}-${Date.now()}.${ext}`);
  },
});

//! so multerFilter definirame kakov tip na fajlovi kje prima nasiot server
...
exports.getAll = async (req, res) => {
  try {
    ...
    // so populate metodata namesto id-inja gi popoulirame so nivnite vrednosti
...
exports.createByUser = async (req, res, next) => {
  try {
    ...
    //! poradi sto imame implementirano express.jwt toj kreira
...
exports.getByUser = async (req, res) => {
  try {
    const userId = req.auth.id;
    //! vrsime query na avtorot od korisnikot sto e logiran
